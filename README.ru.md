...
Стандартная страница выбора папки с подставленным путём и кнопкой «Обзор». Подставляется `%ProgramData%\obs-studio\plugins\obs-auto-capture` — именно там OBS ищет плагины на Windows: `AddExtraModulePaths` вызывает `GetProgramDataPath` и дописывает `bin\64bit` и `data`. `%APPDATA%` — это путь для macOS и Linux, на Windows он не сканируется.

Портативная OBS выходит из этой функции ещё раньше и не смотрит в общую папку вовсе, поэтому для неё подставляется папка самой OBS.
...
Готовые файлы лежат в [релизах](https://github.com/Dimension-Science/obs-auto-capture/releases): установщик и архив с плагином. Обновление OBS установленный плагин не удаляет.
