...
Плагин собирается как in-tree модуль дерева OBS Studio:

```text
obs-studio-32.1.0/obs-studio-32.1.0/plugins/obs-auto-capture
```

Конфигурация и сборка — `run-obs-configure-nmake2.cmd` и `run-obs-build.cmd`. Корневой `CMakeLists.txt` собирает плагин отдельно от дерева OBS при наличии libobs SDK.
